...
import de.dfki.mary.coefficientextraction.extraction.ExtractSTRAIGHT

/**
 *  Definition of the task type to extract spectrum, f0 and aperiodicity using world vocoder
 *
 */
public class ExtractSTRAIGHTTask extends DefaultTask {
    /** The worker */
     ...
     */
    @Inject
    public ExtractSTRAIGHTTask(WorkerExecutor workerExecutor) {
        super();
...
/**
 *  Worker class which extract spectrum, f0 and aperiodicity using the vocoder STRAIGHT
 *
 */
class ExtractSTRAIGHTWorker implements Runnable {
    /** The input SP file */
     ...
     */
    @Inject
    public ExtractSTRAIGHTWorker(File input_file, File sp_output_file, File f0_output_file, File ap_output_file,
    ...
    public void run() {
        ...
        // Define extractor
        def extractor = new ExtractSTRAIGHT(configuration.path.straight);
